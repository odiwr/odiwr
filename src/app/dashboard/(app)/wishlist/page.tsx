...
import WishSection from "@/components/dashboard/WishSection";
 ...
 * already chosen — the same shape as the Work tab. Rows drag to reorder, and
 * each drop saves.
 */
      ...
      {groups.map(({ category, items }) => (
        <section key={category} className="flex flex-col gap-1">
          ...
          {items.length === 0 ? (
            ...
            <WishSection category={category} items={items} />
          )}
        </section>
      ))}
    </div>
